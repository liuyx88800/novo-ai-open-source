...
import { ArrowUpFromLine, Ban, Copy, Eye, Film, GalleryVerticalEnd, Image as ImageIcon, Pencil, Plus, RefreshCw, Scale, Search, Send, Trash2 } from "lucide-react";
...
import { useSearchParams } from "@/studio/next-shim";
import { WorkPublicationEditor } from "./components/work-publication-editor";
import { formatWorkTime, SOURCE_TYPE_LABELS, VISIBILITY_LABELS, workShareHref, workStatusLabel, WORK_STATUS_OPTIONS } from "./work-publication-values";
...
export default function WorksPage() {
    ...
    const submit = async (work: WorkPublication) => {
        ...
        try {
            ...
            message.error(submitError instanceof Error ? submitError.message : "提交发布失败");
        } finally {
            setActionId("");
        }
    };
                    ...
                    <section className="grid min-w-0 gap-2 py-3 sm:gap-3 sm:py-5">
                        {items.map((work) => (
                            <WorkListItem
                                ...
                                onCopy={() => copyText(new URL(workShareHref(work.slug), window.location.origin).toString(), "作品链接已复制")}
                                onPreview={() => window.open(new URL(workShareHref(work.slug), window.location.origin).toString(), "_blank", "noopener,noreferrer")}
                                onAppeal={() => {
                                    setAppealDescription("");
                                    setAppealWork(work);
                                }}
