    ...
    const executeSubmission = useCallback(
        async (snapshot: PendingCreateSubmission) => {
            try {
                ...
                const userMessageId = run.inputMessageId || snapshot.temporaryUserId;
                const assistantMessageId = run.assistantMessageId || snapshot.temporaryAssistantId;
                setMessages((current) =>
                    current.map((item) => {
                        if (item.id === snapshot.temporaryUserId) return { ...item, id: userMessageId, conversationId: run.conversationId, runId: run.id };
                        if (item.id === snapshot.temporaryAssistantId) return { ...item, id: assistantMessageId, conversationId: run.conversationId, runId: run.id };
                        return item;
                    }),
                );
                watchRun(run, assistantMessageId, snapshot.generation);
                return true;
            } catch (error) {
