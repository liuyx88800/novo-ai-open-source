...
import { useParams, useRouter } from "@/studio/next-shim";
...
export default function DramaProjectPage() {
    ...
    useEffect(() => {
        ...
        void loadProject(projectId)
            .then(() => {
                if (!active) return;
                const loaded = useDramaStore.getState().projects.find((item) => item.id === projectId);
                if (loaded && loaded.episodes.length === 0) addEpisode(projectId);
            })
    ...
    }, [loadProject, projectId, userId, addEpisode]);
    if (loading || (project && project.episodes.length === 0)) return <main className="grid h-full place-items-center bg-background text-sm text-muted-foreground">正在加载短剧项目…</main>;
    if (!project)
        return (
